...
// */
const bigSpenders = transactions.filter(transaction => transaction.type === 'sale' && transaction.items.length >= 5).map(transaction => {
...
// */
const sumFirstSale = transactions.filter(transaction => transaction.type === 'sale')[0].items.reduce(((total, item) => total + item.price), 0);
...
const sumPurchases = transactions.filter(transaction => transaction.type === 'purchase').map(transaction => transaction.items).flat().reduce(((total, item) => total + item.price), 0);
...
// */
const netProfit = sumPurchases + transactions.filter(transaction => transaction.type === 'sale').map(transaction => transaction.items).flat().reduce(((total, item) => total + item.price), 0);
